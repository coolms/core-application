...
/**
 * Reads config data with the stored overrides layered over the files.
 *
 ...
 *
 * !! A stored override REPLACES the file rather than merging into it -- a
 ...
 * down. That belongs to whoever implements the port, which is the only layer
 * allowed to name a persistence exception (the platform's ORM-agnostic rule,
 * enforced by a phpstan boundary check). From here a store that cannot answer
 * and a config nobody has overridden are the same thing: null, then the files.
 *
 * !! Read the implementation before widening what it swallows. Catching one
 * exception too many there makes a broken table look exactly like an empty one,
 * and this loader would report nothing wrong while silently ignoring every
 * saved override.
 */
final readonly class ChainedConfigLoader implements ConfigLoaderInterface
{
    public function __construct(
        ...
        private ConfigOverrideReaderInterface $overrides,
    ) {
    }

    public function load(string $type, string $id, ?string $themeSlug = null): ?array
    {
        return $this->overrides->findOverrideData($type, $id)
            ?? $this->files->load($type, $id, $themeSlug);
    }
}
